...
"""
This program considers following folder structure:
.
└── data
    ├── output
    ├── parsed
...
collections_url_list = [
    ...
    (SRG_list, "Strongly Regular"),
    (hypo_hamiltonian_list, "Hypo Hamiltonian"),
    (planar_list, "Planar"),
    (self_complementary, "Self Complementary"),
    (highly_irregular_list, "Highly Irregular"),
...
def download_from_url_list(url_list, collection_name):
    ...
    for url in url_list:
        ...
        subprocess.call(f'{nauty_lib_path}/showg -e ./data/raw/{name} > ./data/showg/{name}', shell=True)
...
def process_graph_list(name_list, collection_name):
    ...
    out_file = f"data/output/{make_valid_name(collection_name)}.txt"
    subprocess.call(f'touch {out_file}', shell=True)
    ...
    for name in name_list:
        ...
        subprocess.call(f'out/check_graphs_all < data/parsed/{name} >> {out_file}', shell=True)
